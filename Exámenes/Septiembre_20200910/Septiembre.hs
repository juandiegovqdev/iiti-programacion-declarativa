...
-- c)
dctRS :: [Float] -> [Float]
dctRS xs = dctRSAux 1 xs

dctRSAux :: Float -> [Float] -> [Float]
dctRSAux _ [] = []
dctRSAux k xs 
    | k <= n    = sumatorioRS 1 k n (xs) : dctRSAux (k+1) xs 
    | otherwise = []
    where n = fromIntegral (length xs)

sumatorioRS :: Float -> Float -> Float -> [Float] -> Float
sumatorioRS _ _ _ [] = 0
sumatorioRS i k n (x:xs)
    | i <= n    = (x * (cos (pi/n*(n+0.5)*(i)*(k)))) + sumatorioRS (i+1) k n xs 
    | otherwise = 0
